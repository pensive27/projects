print("Copyright 2020. Please feel free to use the code. However, please give credit to user under the following Discord application username: SirNeil email: sk992@live.com\n"
	  "Make sure to enter all fields as the programme will prompt renetry for ALL characters otherwise.")
character_listname = []
...
for num in range(characters):
	while True:
		try:
			...
			print("Please enter names using letters only! Please enter all character details")
			continue
		else:
...
for num in range(0, characters):
	while round >= 0:
		round += 1
		print("\nRound ", round)
		print(f"Next is: {character_listname[num]}\nThe AC: {aclist[num]}\nThe HP: {hplist[num]}\ninitiative: {character_listinitiative[num]}\n")
		for postion, number in enumerate(hplist):
			try:
				print(f"Enter health change for {character_listname[num]}:\n current Hp: {number}\n")
				modifier = int(input("Enter health change  (use + 'number',- 'number' or 0 if no change): "))
				result = number + modifier
				hplist[postion] = result
				print("To exit programme simply close.")
			except ValueError:
				print("Please enter only integers and don't leave blanks!")
				continue
